...
checkLosePlayer :-
    ...
    (checkLosePlayer(PlayerCount) -> remove_element(PlayerCount, Order, NewOrder), retractall(turn_order(_)), retractall(player_count(_)), 
    NewPlayerCount is PlayerCount - 1, asserta(player_count(NewPlayerCount)), asserta(turn_order(NewOrder)), true;
    Recursive is PlayerCount - 1, checkLosePlayer(Recursive), true
    ).

checkLosePlayer(PlayerID) :-
    (PlayerID > 0, \+ locationOccupier(PlayerID, _) -> true; fail).
